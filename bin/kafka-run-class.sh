...
if [ "x$DAEMON_MODE" = "xtrue" ]; then
  ...
  exec "$JAVA" -javaagent:/bin/jmx_prometheus_javaagent-0.18.0.jar=localhost:1234:/bin/jmx-kafka-config.yml $KAFKA_HEAP_OPTS $KAFKA_JVM_PERFORMANCE_OPTS $KAFKA_GC_LOG_OPTS $KAFKA_JMX_OPTS $KAFKA_LOG4J_CMD_OPTS -cp "$CLASSPATH" $KAFKA_OPTS "$@"
fi
